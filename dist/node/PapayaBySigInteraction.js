...
const solidity_utils_1 = require("@1inch/solidity-utils");
const ethers_1 = require("ethers");
const permit_1 = require("../helpers/permit");
...
class PapayaBySig {
    ...
    token;
    ChainId;
    constructor(url, chainId, papayaAddress, tokenAddress, secretKey) {
    ...
    async BySigDeposit(amount, deadline, isPermit2 = false) {
        ...
        const signature = await this.wallet.signTypedData({ name: name, version: version, chainId: this.ChainId, verifyingContract: await this.contract.getAddress() }, { SignedCall: [{ name: 'traits', type: 'uint256' }, { name: 'data', type: 'bytes' }] }, callTraits);
        return { callTraits, signature };
    }
    async PermitAndDeposit(amount, deadline) {
        // console.log(this.contract, this.token)
        const permit = await (0, permit_1.createPermit)(this.wallet, this.token, '1', this.ChainId, await this.contract.getAddress(), amount, deadline);
    ...
    async PermitAndBySigDeposit(amount, deadline, isPermit2 = false) {
        ...
        const signature = await this.wallet.signTypedData({ name: name, version: version, chainId: this.ChainId, verifyingContract: await this.contract.getAddress() }, { SignedCall: [{ name: 'traits', type: 'uint256' }, { name: 'data', type: 'bytes' }] }, signedCall);
        // await this.contract.connect(this.wallet).permitAndCall(
        //     ethers.solidityPacked(
        //         ['address', 'bytes'],
    ...
    async buildBySigTraits({ nonceType = 0n, deadline = 0n, relayer = solidity_utils_1.constants.ZERO_ADDRESS.toString(), nonce = 0n, } = {}) {
        if (nonceType > 3) {
...
//# sourceMappingURL=PapayaBySigInteraction.js.map